    ...
    bilingual_str& error)
{
    ...
    for (const auto& input : selected_coins) {
        ...
        if (is_taproot) {
            taproot_keys.emplace_back(sender_secret_key);
        } else {
            plain_keys.emplace_back(sender_secret_key);
        }
    }
    ...
    const auto& smallest_outpoint = std::min_element(tx_outpoints.begin(), tx_outpoints.end());
